...
function Accueil() {
  ...
  return (
    <SiteShell>
      ...
      <section className="bg-background py-8"><div className="mx-auto max-w-[1440px] px-5"><div className="flex items-center gap-8"><TitreSection>Galerie</TitreSection><div className="flex gap-7 text-[10px] font-black uppercase"><Link to="/medias" className="border-b-2 border-primary pb-1 text-primary">Photos</Link><Link to="/medias" className="text-muted-foreground">Vidéos</Link></div><Link to="/medias" className="ml-auto hidden text-[10px] font-bold text-primary md:block">Voir toute la galerie →</Link></div><div className="relative mt-5 grid grid-cols-2 gap-2.5 md:grid-cols-6"><Button asChild size="icon" className="absolute left-0 top-1/2 z-10 h-10 w-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground text-primary-foreground hover:bg-foreground/90" aria-label="Voir la galerie"><Link to="/medias"><ArrowLeft /></Link></Button>{listeGalerie.map((image, index) => <Link key={`${image}-${index}`} to="/medias" className="overflow-hidden rounded-xl"><img src={image} alt={`Galerie Siroco ${index + 1}`} loading="lazy" className="aspect-[1.45/1] h-full w-full object-cover transition-transform hover:scale-105" /></Link>)}<Button asChild size="icon" className="absolute right-0 top-1/2 z-10 h-10 w-10 translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground text-primary-foreground hover:bg-foreground/90" aria-label="Voir la galerie"><Link to="/medias"><ArrowRight /></Link></Button></div></div></section>
    </SiteShell>
  );
}
