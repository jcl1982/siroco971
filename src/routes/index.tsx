...
function Accueil() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <footer id="contact" className="bg-[color-mix(in_oklab,var(--foreground)_94%,black)] text-primary-foreground"><div className="mx-auto grid max-w-[1440px] gap-9 px-7 py-11 md:grid-cols-[1.2fr_.8fr_.8fr_1.2fr_.7fr]"><div><div className="flex items-center gap-3"><img src={logoAsset.url} alt="Logo du Siroco des Abymes" className="h-20 w-20 object-contain" /><div><strong className="font-impact text-2xl uppercase leading-none">Siroco</strong><span className="block text-[11px] font-black uppercase tracking-[0.3em]">Abymes</span><span className="mt-1 block text-[8px] uppercase tracking-[0.25em] opacity-55">Depuis 1979</span></div></div><p className="mt-4 font-hand -rotate-2 text-2xl text-primary-foreground">Passionnément Siroco !</p><div className="mt-1 h-1 w-40 -skew-x-12 rounded-full bg-primary" /></div><div><h2 className="text-xs font-black uppercase tracking-[0.15em]">Liens utiles</h2><div className="mt-4 grid gap-1.5 text-[11px] opacity-75">{navigation.slice(1,7).map(([href,label]) => <a key={href} href={href} className="hover:text-primary">{label}</a>)}</div></div><div><h2 className="text-xs font-black uppercase tracking-[0.15em]">Suivez-nous</h2><div className="mt-4 flex gap-3">{[Facebook,Instagram,Youtube].map((Icon,i) => <span key={i} className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-foreground/10"><Icon className="h-4 w-4" /></span>)}</div></div><div><h2 className="text-xs font-black uppercase tracking-[0.15em]">Newsletter</h2><p className="mt-4 text-[10px] opacity-70">Restez informé de toute l’actualité du club !</p><div className="mt-3 flex h-10 gap-2"><label htmlFor="email" className="sr-only">Votre email</label><input id="email" type="email" placeholder="Votre email" className="min-w-0 flex-1 rounded-lg bg-primary-foreground/10 px-3 text-xs outline-none placeholder:opacity-50" /><Button size="icon" className="btn-3d h-10 w-12 rounded-lg" aria-label="S’inscrire à la newsletter"><ArrowRight /></Button></div></div><div className="border-l-2 border-primary/70 pl-6 font-impact text-lg uppercase leading-tight tracking-[0.2em]">Passion<br />Respect<br />Formation<br />Solidarité</div></div><div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-2 px-7 py-5 text-[9px] opacity-60 sm:flex-row"><span>© {new Date().getFullYear()} Siroco Abymes — Tous droits réservés</span><span>Conçu aux Abymes, en Guadeloupe ♥</span></div></div></footer>
    </div>
  );
}